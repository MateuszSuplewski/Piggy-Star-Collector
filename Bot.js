...
class Bot {
  constructor(options) {
    ...
    this.pathColor = new THREE.Color(0x00ff00);
  ...
  initSounds() {
    const assetsPath = `${this.app.assetsPath}Load/sfx/`;
    this.sfx = new SFX(this.app.camera, assetsPath, this.app.listener);
    this.sfx.load("footsteps", true, 0.3, this.object);
  ...
  seeEnemy() {
    ...
    const intersects = this.raycaster.intersectObjects(this.app.piggy.children);
        ...
        if (this.withinAggroRange()) {
          ...
          if (!this.isFiring) {
            ...
            if (this.isFiring && this.seeEnemy()) {
              ...
              const elapsedTime =
                this.app.clock.getElapsedTime() - this.bulletTime;
              if (elapsedTime > 2.0) this.newPath(this.randomWaypoint);
            }
          }
        } else {
          this.firing = false;
